...
const AddProductPage: React.FC = () => {
  ...
  useEffect(() => {
    if (type) {
      axios.get<APIResponse>(`https://amad.totalh.net/api/requests/requests/getTypeInput.php?typeId=${type}`)
        .then(response => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    axios.post<{ success: boolean; message?: string }>('https://amad.totalh.net/api/requests/requests/addProduct.php', newProduct)
      .then(response => {
        if (response.data.success) {
          navigate('/');
        } else {
